...
// importa o Canvas dinamicamente (só roda no client)
const Canvas = dynamic(
  async () => (await import("@react-three/fiber")).Canvas,
  { ssr: false }
);
const useFrame = (await import("@react-three/fiber")).useFrame;
const useThree = (await import("@react-three/fiber")).useThree;
...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms = useMemo(() => {
    ...
    return {
      u_colors: {
        value: colorsArray.map((color) => color.map((v) => v / 255)),
        type: "uniform3fv",
      },
    ...
    for (const key in uniforms) {
      ...
      switch (u.type) {
        ...
        case "uniform3fv":
          ...
          break;
        default:
          console.warn(`Unknown uniform type for ${key}`);
      }
    }
      ...
      new THREE.ShaderMaterial({
        vertexShader: `
          ...
          void main(){
            ...
            fragCoord = (position.xy + 1.0) * 0.5 * vec2(${window.innerWidth.toFixed(
              1
            )}, ${window.innerHeight.toFixed(1)});
          }`,
